...
# =============================================================================
# 2. CALLBACK 5 SECONDES (Compromis Performance / Feedback)
# =============================================================================
class StreamCallback(BaseCallback):
    def __init__(self, run_id, target_session_timesteps, verbose=0):
        ...
        self.last_time_trigger = time.time()

    def _on_step(self) -> bool:
        # 1. Vérification d'arrêt (Doit être immédiate, à chaque step)
        if training_manager.should_stop(self.run_id):
            return False

            # Capture du point de départ de la session
        if self.initial_steps is None:
            self.initial_steps = self.num_timesteps

        # 2. Mise à jour toutes les 5 secondes
        if time.time() - self.last_time_trigger > 5.0:
            self._send_update()
            self.last_time_trigger = time.time()
    ...
    def _send_update(self):
        try:
            # Calcul des steps faits durant cette session uniquement
